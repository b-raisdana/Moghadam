...
# base_ohlcv_ticks = pd.read_csv(f'{config.files_to_load[0]}.zip', sep=',', header=0, index_col='date',
#                               parse_dates=['date'], skiprows=range(1, 400320), nrows=1440)
# if DEBUG: print(base_ohlcv_ticks)
# # base_ohlcv_ticks['indexer'] = range(0,len(base_ohlcv_ticks))
# base_peaks, base_valleys = find_peaks_n_valleys(base_ohlcv_ticks)
# # base_peaks.to_csv(f'peaks.{base_ohlcv_ticks.index[0].strftime("%y-%m-%d.%H-%M")}T{base_ohlcv_ticks.index[-1].strftime("%y-%m-%d.%H-%M")}.zip')
# # base_valleys.to_csv(f'valleys.{base_ohlcv_ticks.index[0].strftime("%y-%m-%d.%H-%M")}T{base_ohlcv_ticks.index[-1].strftime("%y-%m-%d.%H-%M")}.zip')
#
# # plot_ohlcv_with_peaks_n_valleys(ohlcv=base_ohlcv_ticks, name=f'base_ohlcv_ticks',
...
def test_timeframe_switching():
    for i in range(1, len(config.timeframes)):
        ...
        if timeframe == '1W':
            ...
            frequency = timeframe
        _timeframe_ohlcv_ticks = base_ohlcv_ticks.groupby(pd.Grouper(freq=frequency)) \
            .agg({'open': 'first',
                  'close': 'last',
                  'low': 'min',
                  'high': 'max',
        ...
        try:
            ...
            # plot_ohlcv_with_peaks_n_valleys(ohlcv=base_ohlcv_ticks, name=f'base_ohlcv_ticks  {config.times[i]}',
            #                                peaks=_mapped_peaks_from_base)
            # plot_ohlcv_with_peaks_n_valleys(ohlcv=_timeframe_ohlcv_ticks, name=f'_timeframe_ohlcv_ticks {config.times[i]}',
        ...
        try:
            ...
            # plot_peaks_n_valleys(ohlcv=base_ohlcv_ticks, name=f'base_ohlcv_ticks  {config.timeframes[i]}',
            #                      valleys=_mapped_valleys_from_base)
            # plot_peaks_n_valleys(ohlcv=_timeframe_ohlcv_ticks, name=f'_timeframe_ohlcv_ticks {config.timeframes[i]}',
            #                      peaks=_timeframe_peaks, valleys=_timeframe_valleys)
...
def test_every_peak_is_found():
    for i in range(1, len(base_ohlcv_ticks) - 1):
        if base_ohlcv_ticks.iloc[i - 1]['high'] < \
                base_ohlcv_ticks.iloc[i]['high'] > \
                base_ohlcv_ticks.iloc[i + 1]['high']:
            try:
                assert base_peaks.loc[base_ohlcv_ticks.index[i]]['high'] == base_ohlcv_ticks.iloc[i]['high']
            except Exception as e:
                pass
                log("base_peaks[base_ohlcv_ticks.index[i]]['high']")
                log(base_peaks[base_ohlcv_ticks.index[i]]['high'])
                log("base_ohlcv_ticks.iloc[i]['high']")
                log(base_ohlcv_ticks.iloc[i]['high'])
                raise e
...
def test_strength_of_peaks():
    for _t_index in base_peaks.index:
        right_candles = base_ohlcv_ticks[(_t_index < base_ohlcv_ticks.index) &
                                        (base_ohlcv_ticks.index < (_t_index + base_peaks.loc[_t_index]['strength']))]
        assert base_peaks.loc[_t_index]['high'] >= right_candles.max()['high']

        left_candles = base_ohlcv_ticks[(_t_index - base_peaks.loc[_t_index]['strength'] < base_ohlcv_ticks.index) &
                                       (base_ohlcv_ticks.index < _t_index)]
        assert base_peaks.loc[_t_index]['high'] >= left_candles.max()['high']

        if _t_index - base_peaks.loc[_t_index]['strength'] > base_ohlcv_ticks.index[0]:
            if _t_index + base_peaks.loc[_t_index]['strength'] < base_ohlcv_ticks.index[-1]:
                # the peak is not top in either sides
                assert base_ohlcv_ticks.loc[_t_index + base_peaks.loc[_t_index]['strength']]['high'] > \
                       base_peaks.loc[_t_index]['high'] \
                       or base_ohlcv_ticks.loc[_t_index - base_peaks.loc[_t_index]['strength']]['high'] > \
                       base_peaks.loc[_t_index]['high']
            else:
            ...
            assert base_peaks.loc[_t_index]['strength'] == _t_index - base_ohlcv_ticks.index[0]
