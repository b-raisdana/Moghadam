...
from data_preparation import single_timeframe, concat, cast_and_validate, empty_df
...
def sequence_of_spinning(ohlcva: pt.DataFrame[OHLCVA], timeframe: str, number_of_base_spinning_candles: int = None) \
    ...
    if number_of_base_spinning_candles is None:
        number_of_base_spinning_candles = config.number_of_base_spinning_candles
...
def base_from_sequence(_sequence_of_spinning: pt.DataFrame[OHLCVA]) -> pt.Index[Timestamp]:
    """
    Get the index of rows where, in all columns starting with 'next_candle_overlap_',
    their value is below config.base_candle_min_backward_coverage (80%).
    ...
    is_base = (overlap_columns < config.base_candle_min_backward_coverage).all(axis='columns')
...
def add_sequence_values(ohlcva, high_low: Literal['high', 'low'], number_of_base_spinning_candles: int):
    ohlcva[f'next_candle_{high_low}_0'] = ohlcva[high_low]
    for i in range(number_of_base_spinning_candles):
        ohlcva[f'next_candle_{high_low}_{i}'] = ohlcva[high_low].shift(-i)
    return ohlcva
...
def base_highest_low(base_indexes, ohlcva, number_of_base_spinning_candles: int) -> pt.Series[float]:
    ...
    _base_highest_low = ohlcva.loc[base_indexes].filter(like='next_candle_low_').max(axis='column')
...
def base_lowest_high(base_indexes, ohlcva, number_of_base_spinning_candles: int) -> pt.Series[float]:
    ...
    _base_lowest_high = ohlcva.loc[base_indexes].filter(like='next_candle_high_').min(axis='column')
    return _base_lowest_high


def timeframe_base(ohlcva: pt.DataFrame[OHLCVA], timeframe: str, number_of_base_spinning_candles: int = None) \
    ...
    base_indexes = base_from_sequence(_sequence_of_spinning)
...
def multi_timeframe_base_patterns(multi_timeframe_ohlcva: pt.DataFrame[MultiTimeframeOHLCVA],
    ...
    for timeframe in timeframe_shortlist:
        ...
        _timeframe_bases = timeframe_base(ohlcva, timeframe)
