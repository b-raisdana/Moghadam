...
__all__ = ['ExtendedBroker']


class ExtendedBroker(bt.BrokerBase):
    '''Broker Simulator
    ...
    def __init__(self):
        super(ExtendedBroker, self).__init__()
        self._userhist = []
        self._fundhist = []
    ...
    def init(self):
        super(ExtendedBroker, self).init()
...
from Strategy.ExtendedOrder import ExtendedSellOrder


class zExtendedBroker(bt.BackBroker):
    @staticmethod
    def trigger_order_assertion(**kwargs):
        assert 'trigger_price' in kwargs.keys(), 'Expected to have a trigger_price'
        assert kwargs['trigger_price'] > 0, 'Expected to have a positive trigger_price'
        assert 'limit_price' in kwargs.keys(), 'Expected to have a limit_price'
        assert 'stop_loss_price' in kwargs.keys(), 'Expected to have a stop_loss_price'
        assert 'take_profit_price' in kwargs.keys(), 'Expected to have a take_profit_price'
        return True
    ...
    def buy(self, owner, data,
            ...
            **kwargs):
        # todo: test
        assert self.trigger_order_assertion(**kwargs)
        order = BuyOrder(owner=owner, data=data,
                         size=size, price=price, pricelimit=plimit,
                         exectype=exectype, valid=valid, tradeid=tradeid,
                         trailamount=trailamount, trailpercent=trailpercent,
    ...
    def sell(self, owner, data,
             ...
             **kwargs):
        # todo: test
        assert self.order_kwargs_assertions(**kwargs)
        order = ExtendedSellOrder(
