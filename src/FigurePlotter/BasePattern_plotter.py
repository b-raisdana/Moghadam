...
def plot_single_timeframe_base_pattern(single_timeframe_ohlcva: pt.DataFrame[OHLCV],
    ...
    fig = plot_ohlcva(single_timeframe_ohlcva, name=name, show=False, save=False)
    ...
    for _start, base_pattern in timeframe_base_patterns.iterrows():
        ...
        if remained_number_of_scatters > 0:
            fig.add_scatter(x=xs, y=ys, fill="toself",  # fillcolor=fill_color,
                            ...
                            mode='lines',  # +text',
                            legendgroup=f'{base_pattern["bull_bear_side"].replace("_TREND", "")}: '
                                        f'{_start.strftime("%H:%M")}-{base_pattern["end"].strftime("%H:%M")}',
                            )
        ...
        if 'movement_start_value' in boundaries.columns:
            fig.add_scatter(x=[base_pattern['movement_start_time'], base_pattern['movement_end_time']],
                            y=[base_pattern['movement_start_value'], base_pattern['movement_end_value']],
...
@measure_time
def plot_multi_timeframe_base_pattern(multi_timeframe_ohlcva, multi_timeframe_peaks_n_valleys,
